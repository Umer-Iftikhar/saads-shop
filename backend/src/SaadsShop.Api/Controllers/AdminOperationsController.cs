...
public sealed class AdminInventoryController(
    IOperationsQueryService reads,
    IOperationsCommandService writes) : ApiControllerBase
{
    [HttpGet]
    public async Task<IActionResult> Get([FromQuery] InventorySearchQuery query, CancellationToken ct)
        => FromResult(await reads.GetInventoryAsync(query, ct));
    ...
    public async Task<IActionResult> Adjust(
        ...
        => FromResult(await writes.AdjustStockAsync(productId, request, CurrentUserId, ct));
}

/// <summary>The stitching floor's board.</summary>
[Route("api/admin/stitching-queue")]
[Authorize(Policy = AuthPolicies.StaffOnly)]
public sealed class AdminStitchingController(
    IOperationsQueryService reads,
    IOperationsCommandService writes) : ApiControllerBase
{
    [HttpGet]
    public async Task<IActionResult> Get(CancellationToken ct)
        => FromResult(await reads.GetStitchingBoardAsync(ct));

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] StitchingJobCreateRequest request, CancellationToken ct)
        => FromResult(await writes.CreateStitchingJobAsync(request, ct), StatusCodes.Status201Created);

    [HttpPatch("{jobId:int}")]
    public async Task<IActionResult> Update(
        int jobId, [FromBody] StitchingJobUpdateRequest request, CancellationToken ct)
        => FromResult(await writes.UpdateStitchingJobAsync(jobId, request, ct),
...
[Authorize(Policy = AuthPolicies.StaffOnly)]
public sealed class AdminCustomersController(IOperationsQueryService operations) : ApiControllerBase
{
    [HttpGet]
