    ...
    IOrderCommandService writes) : ApiControllerBase
{
    ...
    public async Task<IActionResult> Search([FromQuery] OrderSearchQuery query, CancellationToken ct)
        => FromResult(await reads.SearchAsync(query, ct));

    [HttpGet("{id:int}")]
    public async Task<IActionResult> Get(int id, CancellationToken ct)
        => FromResult(await reads.GetAsync(id, ct));

    [HttpPatch("{id:int}/status")]
    public async Task<IActionResult> UpdateStatus(
        int id, [FromBody] UpdateOrderStatusRequest request, CancellationToken ct)
        => FromResult(await writes.UpdateStatusAsync(id, request, CurrentUserId, ct),
    ...
    public async Task<IActionResult> SaveMeasurements(
        ...
        => FromResult(await writes.SaveMeasurementsAsync(id, request, CurrentUserId, ct),
                      StatusCodes.Status204NoContent);
}
