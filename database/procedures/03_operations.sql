...
BEGIN
    ...
    BEGIN CATCH
        ...
        SELECT @ResponseCode = 500, @ResponseMessage = N'Could not load inventory. Please try again.';
    END CATCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        ELSE IF @Delta IS NULL OR @Delta = 0
            SELECT @ResponseCode = 400, @ResponseMessage = N'Enter how many pieces to add or remove.';
        ELSE IF ABS(@Delta) > 100000
            SELECT @ResponseCode = 400, @ResponseMessage = N'That adjustment is too large.';
        ELSE IF NULLIF(LTRIM(RTRIM(@Reason)), N'') IS NULL
            SELECT @ResponseCode = 400, @ResponseMessage = N'Give a reason for the adjustment.';
        ELSE IF LEN(@Reason) > 200
            SELECT @ResponseCode = 400, @ResponseMessage = N'That reason is too long.';
    ...
    BEGIN CATCH
        ...
        INSERT INTO dbo.ErrorLog (ProcedureName, ErrorNumber, ErrorMessage, ErrorLine, ErrorSeverity)
        VALUES (ERROR_PROCEDURE(), ERROR_NUMBER(), ERROR_MESSAGE(), ERROR_LINE(), ERROR_SEVERITY());
        SELECT @ResponseCode = 500, @ResponseMessage = N'Could not update stock. Please try again.';
    END CATCH
...
BEGIN
    ...
    BEGIN CATCH
        INSERT INTO dbo.ErrorLog (ProcedureName, ErrorNumber, ErrorMessage, ErrorLine, ErrorSeverity)
        VALUES (ERROR_PROCEDURE(), ERROR_NUMBER(), ERROR_MESSAGE(), ERROR_LINE(), ERROR_SEVERITY());
        SELECT @ResponseCode = 500, @ResponseMessage = N'Could not load the stitching queue. Please try again.';
    END CATCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        ELSE IF NULLIF(LTRIM(RTRIM(@Title)), N'') IS NULL
            SELECT @ResponseCode = 400, @ResponseMessage = N'Give the job a description.';
        ELSE IF LEN(@Title) > 160
            SELECT @ResponseCode = 400, @ResponseMessage = N'That description is too long.';
        ...
        ELSE IF @DueDate IS NOT NULL AND @DueDate < DATEADD(YEAR, -1, CAST(SYSUTCDATETIME() AS DATE))
            SELECT @ResponseCode = 400, @ResponseMessage = N'That due date is not sensible.';
        ELSE IF @OrderLineId IS NOT NULL
            ...
            SELECT @ResponseCode = 400, @ResponseMessage = N'That item is not on this order.';
    ...
    BEGIN CATCH
        ...
        INSERT INTO dbo.ErrorLog (ProcedureName, ErrorNumber, ErrorMessage, ErrorLine, ErrorSeverity)
        VALUES (ERROR_PROCEDURE(), ERROR_NUMBER(), ERROR_MESSAGE(), ERROR_LINE(), ERROR_SEVERITY());
        SELECT @ResponseCode = 500, @ResponseMessage = N'Could not add that job. Please try again.';
    END CATCH
...
BEGIN
    ...
    BEGIN TRY
        IF @StitchingJobId IS NULL OR @StitchingJobId <= 0
            SELECT @ResponseCode = 400, @ResponseMessage = N'Job id is required.';
        ELSE IF @Stage IS NOT NULL AND @Stage NOT IN (N'Measuring', N'Cutting', N'Stitching', N'Ready', N'Done')
            SELECT @ResponseCode = 400, @ResponseMessage = N'That is not a stage on the floor.';
        ELSE IF @Stage IS NULL AND @AssignedTo IS NULL AND @DueDate IS NULL AND @ClearDueDate = 0
            SELECT @ResponseCode = 400, @ResponseMessage = N'Nothing to change.';
        ...
        BEGIN
            BEGIN TRANSACTION;
                ...
                BEGIN
                    ROLLBACK TRANSACTION;
                    SELECT @ResponseCode = 404, @ResponseMessage = N'That job no longer exists.';
                END
                ELSE
                BEGIN
    ...
    BEGIN CATCH
        ...
        INSERT INTO dbo.ErrorLog (ProcedureName, ErrorNumber, ErrorMessage, ErrorLine, ErrorSeverity)
        VALUES (ERROR_PROCEDURE(), ERROR_NUMBER(), ERROR_MESSAGE(), ERROR_LINE(), ERROR_SEVERITY());
        SELECT @ResponseCode = 500, @ResponseMessage = N'Could not update the job. Please try again.';
    END CATCH
...
BEGIN
    ...
    BEGIN CATCH
        ...
        INSERT INTO dbo.ErrorLog (ProcedureName, ErrorNumber, ErrorMessage, ErrorLine, ErrorSeverity)
        VALUES (ERROR_PROCEDURE(), ERROR_NUMBER(), ERROR_MESSAGE(), ERROR_LINE(), ERROR_SEVERITY());
        SELECT @ResponseCode = 500, @ResponseMessage = N'Could not load customers. Please try again.';
    END CATCH
